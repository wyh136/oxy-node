...
before(function (done) {
	// Send coins to account 1 address
	setTimeout(function () {
...
before(function (done) {
	// Send coins to account 2 address
	setTimeout(function () {
		var randomOXY = node.randomOXY();
		var expectedFee = node.expectedFee(randomOXY);
...
describe('PUT /dapps', function () {
	...
	it('using account with no funds should fail', function (done) {
		...
		node.put('/api/dapps', validParams, function (err, res) {
			...
			node.expect(res.body).to.have.property('error').to.match(/Account does not have enough OXY: [0-9]+L balance: 0/);
			done();
		});
	});
