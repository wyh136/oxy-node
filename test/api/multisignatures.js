...
function sendOXY (account, i, done) {
	var randomOXY = node.randomOXY();
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomOXY / node.normalizer;
		}
		done();
	});
}

function sendOXYFromMultisigAccount (amount, recipient, done) {
	node.put('/api/transactions/', {
...
before(function (done) {
	sendOXY(multisigAccount, null, done);
});
...
describe('PUT /api/multisignatures', function () {
	...
	it('using random passphase should fail', function (done) {
		...
		node.put('/api/multisignatures', validParams, function (err, res) {
			...
			node.expect(res.body).to.have.property('error').to.match(/Account does not have enough OXY: [0-9]+X balance: 0/);
			done();
		});
	});
...
describe('PUT /api/transactions', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendOXYFromMultisigAccount(100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
