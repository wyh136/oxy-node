...
describe('POST /peer/transactions', function () {
	...
	describe('registering a delegate', function () {
		...
		it('using undefined transaction.asset', function (done) {
			var transaction = node.shift.delegate.createDelegate(node.randomPassword(), node.randomDelegateName().toLowerCase());
		...
		describe('when account has funds', function () {
			...
			it('using invalid username should fail', function (done) {
				var transaction = node.shift.delegate.createDelegate(account.password, crypto.randomBytes(64).toString('hex'));
				transaction.fee = node.fees.delegateRegistrationFee;
			...
			it('using uppercase username should fail', function (done) {
				...
				var transaction = node.shift.delegate.createDelegate(account.password, account.username);
			...
			describe('when lowercased username already registered', function () {
				it('using uppercase username should fail', function (done) {
					var transaction = node.shift.delegate.createDelegate(account2.password, account.username.toUpperCase());
		...
		describe('twice for the same account', function () {
			...
			it('should fail', function (done) {
				...
				var transaction = node.shift.delegate.createDelegate(account2.password, account2.username);

				account2.username = node.randomDelegateName().toLowerCase();
				var transaction2 = node.shift.delegate.createDelegate(account2.password, account2.username);
