...
describe('POST /peer/transactions', function () {
	...
	it('using varying recipientId casing should go to same address', function (done) {
		...
		var address = node.shift.crypto.getAddress(keys.publicKey);

		var transaction = node.shift.transaction.createTransaction(address, 100000000, node.gAccount.password);
		postTransaction(transaction, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.ok;

			node.onNewBlock(function (err) {
				var transaction2 = node.shift.transaction.createTransaction(address.toLowerCase(), 100000000, node.gAccount.password);
	...
	it('using transaction with undefined recipientId should fail', function (done) {
		var transaction = node.shift.transaction.createTransaction(undefined, 1, node.gAccount.password);
	...
	it('using transaction with negative amount should fail', function (done) {
		var transaction = node.shift.transaction.createTransaction('1L', -1, node.gAccount.password);
	...
	it('using invalid passphrase should fail', function (done) {
		var transaction = node.shift.transaction.createTransaction('12L', 1, node.gAccount.password);
	...
	it('when sender has no funds should fail', function (done) {
		var transaction = node.shift.transaction.createTransaction('1L', 1, 'randomstring');

		postTransaction(transaction, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.not.ok;
			node.expect(res.body).to.have.property('message').to.match(/Account does not have enough OXY: [0-9]+X balance: 0/);
			done();
		});
	});
	...
	it('when sender does not have enough funds should always fail', function (done) {
		...
		postTransaction(transaction, function (err, res) {
			...
			node.onNewBlock(function () {
				var count = 1;
				var transaction2 = node.shift.transaction.createTransaction(node.gAccount.address, 2, account.password);

				node.async.doUntil(function (next) {
					postTransaction(transaction2, function (err, res) {
						node.expect(res.body).to.have.property('success').to.be.not.ok;
						node.expect(res.body).to.have.property('message').to.match(/Account does not have enough OXY: [0-9]+X balance: 1e-8/);
						count++;
						return next();
					});
				}, function () {
	...
	it('using very large amount and genesis block id should fail', function (done) {
		var transaction = node.shift.transaction.createTransaction('12L', 10000000000000000, node.gAccount.password);
	...
	it('using overflown amount should fail', function (done) {
		var transaction = node.shift.transaction.createTransaction('12L', 184819291270000000012910218291201281920128129,
	...
	it('using float amount should fail', function (done) {
		var transaction = node.shift.transaction.createTransaction('12L', 1.3, node.gAccount.password);
