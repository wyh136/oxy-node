...
function sendOXY (account, amount, done) {
...
before(function (done) {
	setTimeout(function () {
    sendOXY(account, node.randomOXY(), done);
	}, 2000);
});

before(function (done) {
	setTimeout(function () {
		sendOXY(account2, node.randomOXY(), done);
	}, 2000);
});
...
before(function (done) {
	setTimeout(function () {
		// Send 20 SHF
		sendOXY(account2, 20*100000000, done);
	}, 2000);
});
...
before(function (done) {
	setTimeout(function () {
		// Send 100 SHF
		sendOXY(account2, 100*100000000, done);
	}, 2000);
});
	...
	it('using entire balance should fail', function (done) {
		openAccount({ secret: account.password }, function (err, res) {
			...
			putTransaction({
				...
				node.expect(res.body).to.have.property('success').to.be.not.ok;
				node.expect(res.body).to.have.property('error').to.match(/Account does not have enough OXY: [0-9]+X balance: [0-9.]+/);
				done();
			});
		});
	});
